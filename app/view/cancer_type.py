...
class CancerTypeList(ListView):
    """ the process profile list page (operation). """
    login_url = '/login/'
    model = CancerType
    template_name = 'app/cancer_type/box_position_list.html'
    paginate_by = 10
    # permission_required = 'oven.view_process_profile'

    def get_context_data(self, *args, **kwargs):
        context = super().get_context_data(*args, **kwargs)
        context['cancer_type'] = CancerType.objects.all()

        cancer_type = self.request.GET.get('cancer_type', None)
        context['search_condition'] = f'&sample_type={cancer_type}'
        return context

    def get_queryset(self):
        cancer_type = self.request.GET.get('cancer_type', None)
        if cancer_type == '' or cancer_type is None:
            return CancerType.objects.all()
        ct = CancerType.objects\
            .filter(name__icontains=cancer_type)
        return ct
...
def update(request):
    ...
    try:
        ...
        st = CancerType.objects\
            .get(id=data['id'])
        st.name = data['cancer_type']
